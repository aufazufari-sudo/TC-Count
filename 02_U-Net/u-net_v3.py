...
Input geopotential-height files used to derive 300-500 hPa thickness:
    Data_M1/HGT500/HGT500.YYYY_6hourly.nc
    Data_M1/HGT300/HGT300.YYYY_6hourly.nc

Input label files:
    IBTrACS/LABELS.YYYY_6hourly.nc

Assumptions:
    - PRMSL, HGT500, HGT300, and LABELS files share time/lat/lon coordinates.
    - Label variable is 'tc_label', as produced by the uploaded Gaussian-label script.
    - Pressure / geopotential-height variables are auto-detected unless explicit
...
Default behaviour:
    ...
    - Input: pressure + 300-500 hPa geopotential thickness by default.
...
def infer_gridded_variable(ds: xr.Dataset, time_name: str, lat_name: str, lon_name: str) -> str:
    """
    Infer a gridded predictor variable as the first data variable with dimensions
    including time/lat/lon. Falls back to the first data variable if necessary.
    """
...
class HurricaneHeatmapDataset(Dataset):
    """
    Lazy-loading dataset for PRMSL, optional 300-500 hPa geopotential thickness,
    and Gaussian hurricane labels.

    Each item returns:
        x: torch.float32, shape (C, H, W)
    ...
    def __init__(
        ...
        prmsl_files: Dict[int, Path],
        label_files: Dict[int, Path],
        hgt500_files: Optional[Dict[int, Path]] = None,
        hgt300_files: Optional[Dict[int, Path]] = None,
        pressure_var: Optional[str] = None,
        hgt500_var: Optional[str] = None,
        hgt300_var: Optional[str] = None,
        label_var: str = "tc_label",
        input_mode: str = "pressure_thickness",
        time_name: str = "time",
        lat_name: str = "lat",
        lon_name: str = "lon",
        ...
        self.hgt500_files = hgt500_files or {}
        self.hgt300_files = hgt300_files or {}
        self.pressure_var = pressure_var
        self.hgt500_var = hgt500_var
        self.hgt300_var = hgt300_var
        ...
        if self.input_mode not in {"pressure", "pressure_thickness"}:
            raise ValueError(
                f"Unsupported input_mode={self.input_mode!r}. "
                "Choose from {'pressure', 'pressure_thickness'}."
            )

        self._pressure_ds_cache: Dict[int, xr.Dataset] = {}
        self._hgt500_ds_cache: Dict[int, xr.Dataset] = {}
        self._hgt300_ds_cache: Dict[int, xr.Dataset] = {}
        self._label_ds_cache: Dict[int, xr.Dataset] = {}
        self._pressure_var_by_year: Dict[int, str] = {}
        self._hgt500_var_by_year: Dict[int, str] = {}
        self._hgt300_var_by_year: Dict[int, str] = {}
        self._pressure_norm_stats_by_year: Dict[int, Tuple[float, float]] = {}
        self._thickness_norm_stats_by_year: Dict[int, Tuple[float, float]] = {}

        self._initialise_year_metadata()

    @property
    def num_input_channels(self) -> int:
        return 2 if self.input_mode == "pressure_thickness" else 1
    ...
    def _open_hgt500_ds(self, year: int) -> xr.Dataset:
        if self.cache_open_datasets:
            if year not in self._hgt500_ds_cache:
                self._hgt500_ds_cache[year] = xr.open_dataset(self.hgt500_files[year])
            return self._hgt500_ds_cache[year]
        return xr.open_dataset(self.hgt500_files[year])

    def _open_hgt300_ds(self, year: int) -> xr.Dataset:
        if self.cache_open_datasets:
            if year not in self._hgt300_ds_cache:
                self._hgt300_ds_cache[year] = xr.open_dataset(self.hgt300_files[year])
            return self._hgt300_ds_cache[year]
        return xr.open_dataset(self.hgt300_files[year])
    ...
    def _maybe_convert_longitude_to_reference(self, da: xr.DataArray, ref_lon: xr.DataArray) -> xr.DataArray:
        if self.lon_name not in da.coords or self.lon_name not in ref_lon.coords:
            return da

        da_lon = np.asarray(da[self.lon_name].values)
        ref_lon_vals = np.asarray(ref_lon[self.lon_name].values)
        if da_lon.size == 0 or ref_lon_vals.size == 0:
            return da

        da_uses_360 = np.nanmax(da_lon) > 180.0
        ref_uses_360 = np.nanmax(ref_lon_vals) > 180.0
        if da_uses_360 and not ref_uses_360:
            da = da.assign_coords(
                {self.lon_name: (((da[self.lon_name] + 180.0) % 360.0) - 180.0)}
            )
            da = da.sortby(self.lon_name)
        elif ref_uses_360 and not da_uses_360:
            da = da.assign_coords(
                {self.lon_name: (da[self.lon_name] % 360.0)}
            )
            da = da.sortby(self.lon_name)
        return da

    def _require_dims(self, da: xr.DataArray, required: Sequence[str], field_name: str) -> None:
        missing = [d for d in required if d not in da.dims]
        if missing:
            raise ValueError(
                f"{field_name} is missing required dims {missing}; got dims={da.dims}"
            )

    def _drop_unexpected_singletons(
        self,
        da: xr.DataArray,
        allowed_dims: Sequence[str],
        field_name: str,
    ) -> xr.DataArray:
        allowed = set(allowed_dims)
        for dim in list(da.dims):
            if dim not in allowed:
                if da.sizes.get(dim, None) == 1:
                    da = da.squeeze(dim, drop=True)
                else:
                    raise ValueError(
                        f"{field_name} has unsupported non-singleton dim {dim!r} with size {da.sizes[dim]}; dims={da.dims}"
                    )
        return da

    def _to_2d_on_reference_grid(
        self,
        da: xr.DataArray,
        ref_2d: xr.DataArray,
        field_name: str,
    ) -> xr.DataArray:
        da = self._drop_unexpected_singletons(
            da,
            allowed_dims=[self.lat_name, self.lon_name],
            field_name=field_name,
        )
        self._require_dims(da, [self.lat_name, self.lon_name], field_name)
        da = self._maybe_convert_longitude_to_reference(da, ref_2d)
        da = da.transpose(self.lat_name, self.lon_name)

        ref_2d = self._drop_unexpected_singletons(
            ref_2d,
            allowed_dims=[self.lat_name, self.lon_name],
            field_name=f"{field_name}_reference",
        )
        self._require_dims(ref_2d, [self.lat_name, self.lon_name], f"{field_name}_reference")
        ref_2d = ref_2d.transpose(self.lat_name, self.lon_name)

        same_lat = np.array_equal(
            np.asarray(da[self.lat_name].values),
            np.asarray(ref_2d[self.lat_name].values),
        )
        same_lon = np.array_equal(
            np.asarray(da[self.lon_name].values),
            np.asarray(ref_2d[self.lon_name].values),
        )
        if not (same_lat and same_lon):
            da = da.interp(
                {
                    self.lat_name: ref_2d[self.lat_name],
                    self.lon_name: ref_2d[self.lon_name],
                },
                method="linear",
            )
        return da.astype(np.float32)

    def _to_3d_on_reference_grid(
        self,
        da: xr.DataArray,
        ref_3d: xr.DataArray,
        field_name: str,
        ...
        da = self._drop_unexpected_singletons(
            da,
            allowed_dims=[self.time_name, self.lat_name, self.lon_name],
            field_name=field_name,
        )
        self._require_dims(da, [self.time_name, self.lat_name, self.lon_name], field_name)
        da = self._maybe_convert_longitude_to_reference(da, ref_3d)
        da = da.transpose(self.time_name, self.lat_name, self.lon_name)

        ref_3d = self._drop_unexpected_singletons(
            ref_3d,
            allowed_dims=[self.time_name, self.lat_name, self.lon_name],
            field_name=f"{field_name}_reference",
        )
        self._require_dims(ref_3d, [self.time_name, self.lat_name, self.lon_name], f"{field_name}_reference")
        ref_3d = ref_3d.transpose(self.time_name, self.lat_name, self.lon_name)

        same_lat = np.array_equal(
            np.asarray(da[self.lat_name].values),
            np.asarray(ref_3d[self.lat_name].values),
        )
        same_lon = np.array_equal(
            np.asarray(da[self.lon_name].values),
            np.asarray(ref_3d[self.lon_name].values),
        )
        if not (same_lat and same_lon):
            da = da.interp(
                {
                    self.lat_name: ref_3d[self.lat_name],
                    self.lon_name: ref_3d[self.lon_name],
                },
                method="linear",
            )
        return da.astype(np.float32)
    ...
    def _initialise_year_metadata(self) -> None:
        ...
        for year in years:
            with xr.open_dataset(self.prmsl_files[year]) as pds:
                pvar = self.pressure_var or infer_gridded_variable(
                    pds, self.time_name, self.lat_name, self.lon_name
                )
                self._pressure_var_by_year[year] = pvar

                pressure_full = self._to_3d_on_reference_grid(
                    pds[pvar],
                    pds[pvar],
                    field_name=f"PRMSL[{year}]",
                ).load()

                if self.normalize == "zscore_per_year":
                    parr = pressure_full.values.astype(np.float32)
                    pmean = float(np.nanmean(parr))
                    pstd = float(np.nanstd(parr))
                    if not np.isfinite(pstd) or pstd == 0:
                        pstd = 1.0
                    self._pressure_norm_stats_by_year[year] = (pmean, pstd)

            if self.input_mode == "pressure_thickness":
                if year not in self.hgt500_files or year not in self.hgt300_files:
                    raise FileNotFoundError(
                        f"input_mode='pressure_thickness' requires both HGT500 and HGT300 files for year {year}."
                    )

                with xr.open_dataset(self.hgt500_files[year]) as ds500, xr.open_dataset(self.hgt300_files[year]) as ds300:
                    hvar500 = self.hgt500_var or infer_gridded_variable(
                        ds500, self.time_name, self.lat_name, self.lon_name
                    )
                    hvar300 = self.hgt300_var or infer_gridded_variable(
                        ds300, self.time_name, self.lat_name, self.lon_name
                    )
                    self._hgt500_var_by_year[year] = hvar500
                    self._hgt300_var_by_year[year] = hvar300

                    if self.normalize == "zscore_per_year":
                        h500_full = self._to_3d_on_reference_grid(
                            ds500[hvar500],
                            pressure_full,
                            field_name=f"HGT500[{year}]",
                        )
                        h300_full = self._to_3d_on_reference_grid(
                            ds300[hvar300],
                            pressure_full,
                            field_name=f"HGT300[{year}]",
                        )
                        thickness = (h300_full - h500_full).values.astype(np.float32)
                        tmean = float(np.nanmean(thickness))
                        tstd = float(np.nanstd(thickness))
                        if not np.isfinite(tstd) or tstd == 0:
                            tstd = 1.0
                        self._thickness_norm_stats_by_year[year] = (tmean, tstd)

    @staticmethod
    def _zscore(field: np.ndarray, mean: float, std: float) -> np.ndarray:
        if not np.isfinite(std) or std == 0:
            std = 1.0
        return (field - mean) / std

    @staticmethod
    def _zscore_per_sample(field: np.ndarray) -> np.ndarray:
    ...
    def __getitem__(self, idx: int):
        ...
        pressure_da = self._to_2d_on_reference_grid(
            pds[pvar].isel({self.time_name: time_index}),
            pds[pvar].isel({self.time_name: time_index}),
            field_name=f"PRMSL[{year}, t={time_index}]",
        )
        channels = [pressure_da.values.astype(np.float32)]

        if self.input_mode == "pressure_thickness":
            ds500 = self._open_hgt500_ds(year)
            ds300 = self._open_hgt300_ds(year)
            hvar500 = self._hgt500_var_by_year[year]
            hvar300 = self._hgt300_var_by_year[year]

            h500_da = self._to_2d_on_reference_grid(
                ds500[hvar500].isel({self.time_name: time_index}),
                pressure_da,
                field_name=f"HGT500[{year}, t={time_index}]",
            )
            h300_da = self._to_2d_on_reference_grid(
                ds300[hvar300].isel({self.time_name: time_index}),
                pressure_da,
                field_name=f"HGT300[{year}, t={time_index}]",
            )
            thickness = (h300_da - h500_da).values.astype(np.float32)
        ...
        if self.normalize == "zscore_per_year":
            mean, std = self._pressure_norm_stats_by_year[year]
            channels[0] = self._zscore(channels[0], mean, std)
            if self.input_mode == "pressure_thickness":
                mean, std = self._thickness_norm_stats_by_year[year]
                channels[1] = self._zscore(channels[1], mean, std)
        elif self.normalize == "zscore_per_sample":
            channels = [self._zscore_per_sample(ch) for ch in channels]
        elif self.normalize == "none":
            pass
        else:
            raise ValueError(f"Unknown normalization: {self.normalize}")

        shape_set = {tuple(ch.shape) for ch in channels}
        if len(shape_set) != 1:
            raise ValueError(
                f"Channel shape mismatch at year={year}, time_index={time_index}: "
                f"{[tuple(ch.shape) for ch in channels]}"
            )

        x = np.stack(
            [np.nan_to_num(ch, nan=0.0, posinf=0.0, neginf=0.0) for ch in channels],
            axis=0,
        )
        ...
        lat = np.asarray(pressure_da[self.lat_name].values, dtype=np.float32)
        lon = np.asarray(pressure_da[self.lon_name].values, dtype=np.float32)
    ...
    def close(self) -> None:
        for ds in self._pressure_ds_cache.values():
            ds.close()
        for ds in self._hgt500_ds_cache.values():
            ds.close()
        for ds in self._hgt300_ds_cache.values():
            ds.close()
        for ds in self._label_ds_cache.values():
            ds.close()
        self._pressure_ds_cache.clear()
        self._hgt500_ds_cache.clear()
        self._hgt300_ds_cache.clear()
...
def run_cross_validation(
    ...
    prmsl_files: Dict[int, Path],
    label_files: Dict[int, Path],
    hgt500_files: Dict[int, Path],
    hgt300_files: Dict[int, Path],
    ...
    config.update({
        ...
        "cv_fold_years": folds,
        "cv_mode": True,
        "num_input_channels": 2 if args.input_mode == "pressure_thickness" else 1,
    })
    ...
    for fold_idx, holdout_years in enumerate(folds, start=1):
        ...
        train_ds = HurricaneHeatmapDataset(
            ...
            prmsl_files,
            label_files,
            hgt500_files=hgt500_files,
            hgt300_files=hgt300_files,
            pressure_var=args.pressure_var,
            hgt500_var=args.hgt500_var,
            hgt300_var=args.hgt300_var,
        ...
        model = UNet2D(
            in_channels=train_ds.num_input_channels,
            out_channels=1,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a U-Net hurricane detector from PRMSL, 300-500 hPa thickness, and Gaussian IBTrACS labels."
    )
    ...
    parser.add_argument("--hgt500-pattern", default="Data_M1/HGT500/HGT500.*_6hourly.nc")
    parser.add_argument("--hgt300-pattern", default="Data_M1/HGT300/HGT300.*_6hourly.nc")

    parser.add_argument("--pressure-var", default=None)
    parser.add_argument("--hgt500-var", default=None)
    parser.add_argument("--hgt300-var", default=None)
    parser.add_argument("--label-var", default="tc_label")
    parser.add_argument(
        "--input-mode",
        choices=["pressure", "pressure_thickness"],
        default="pressure_thickness",
        help=(
            "Input composition: PRMSL only, or PRMSL plus 300-500 hPa geopotential thickness "
            "(HGT300 - HGT500)."
        ),
    )
    ...
    hgt500_files = build_file_map(args.hgt500_pattern)
    hgt300_files = build_file_map(args.hgt300_pattern)

    available_years = sorted(set(prmsl_files).intersection(label_files))
    if args.input_mode == "pressure_thickness":
        available_years = sorted(set(available_years).intersection(hgt500_files).intersection(hgt300_files))
    if not available_years:
        required = "PRMSL/LABELS/HGT500/HGT300" if args.input_mode == "pressure_thickness" else "PRMSL/LABELS"
        raise RuntimeError(f"No matching {required} year files found.")
