...
def run_cross_validation(
    ...
    for fold_idx, holdout_years in enumerate(folds, start=1):
        ...
        if args.resume_cv and fold_outputs_exist(fold_out_dir):
            ...
            diag_df = pd.read_csv(fold_out_dir / "holdout_diagnostics.csv")
            det_df = pd.read_csv(fold_out_dir / "peak_detections.csv")

            if "fold" not in diag_df.columns:
                diag_df.insert(0, "fold", fold_idx)
            if "split" not in diag_df.columns:
                diag_df.insert(1, "split", "holdout")

            if "fold" not in det_df.columns:
                det_df.insert(0, "fold", fold_idx)
            if "split" not in det_df.columns:
